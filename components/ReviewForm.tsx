    ...
    <section className="rounded-2xl border border-line bg-card p-5">
      ...
      {editing && (
        <p className="mt-2 rounded-xl bg-accent-soft/40 px-3 py-2 text-xs leading-5 text-accent-strong">
          <Icon name="gift" /> 후기를 남겨주시면 <b>올해 운세 리포트 1회 무료</b> 쿠폰을 드려요.
          <br />
        ...
        {[1, 2, 3, 4, 5].map((n) => (
          <button
            ...
            className={`text-2xl transition ${
              n <= rating ? "text-[#9A7420]" : "text-[#8A8398]"
            } ${editing ? "hover:scale-110" : "cursor-default"}`}
          >
            {/* 크림 카드 위 연한 금색은 대비가 낮아 채움 여부를 모양(★/☆)으로도 구분 */}
            {n <= rating ? "★" : "☆"}
          </button>
        ))}
      </div>
      ...
      {editing ? (
        <>
          ...
          <button
            ...
            className="mt-2 min-h-11 w-full rounded-xl bg-[#272132] px-4 py-2.5 text-sm font-bold text-[#FFE9A8] transition hover:opacity-90 disabled:opacity-50"
          >
          ...
          {coupon && (
            <div className="mt-3 rounded-xl border-2 border-accent bg-accent-soft/30 p-3.5">
              <p className="text-xs font-bold text-accent-strong">
                <Icon name="gift" /> {coupon.benefit} 쿠폰
              </p>
